...
public class BusController {
	...
	public String home1(Model m, HttpSession session) {
		String movie = (String) session.getAttribute("moviename");
		System.out.println(movie + "========Index");
		List<UpcommingMovies> movie3 = dao.getAllMovies();
		m.addAttribute("movieList1", movie3);
		m.addAttribute("menu", "booksoon");
		return "newmovie";
	}

	
	@GetMapping("/casting")
	public String cast(/* Model m, HttpSession session */) {
		/*
		 * String movie = (String) session.getAttribute("actorName");
		 * System.out.println(movie + "========Index"); List<ActorDetails> movie3 =
		 * dao.getAllActors(); m.addAttribute("actorlist", movie3);
		 * m.addAttribute("menu", "casting");
		 */
		return "actor";
	}
...
//		Login process
	@PostMapping("/processing") // Into Login.html
